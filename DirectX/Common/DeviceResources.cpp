...
{
	// Дисплеи высокого разрешения могут быть более требовательны к GPU и быстрее расходовать батарею при отрисовке.
...
void DX::DeviceResources::CreateWindowSizeDependentResources()
{
	...
	// Очистить содержимое, связанное с размером предыдущего окна.
	for (UINT n = 0; n < c_frameCount; n++)
	{
	...
	else
	{
		...
		swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;	// Все универсальный приложения Windows должны использовать _FLIP_ SwapEffects
	...
	// Создайте представление трафарета глубины.
	{
	...
	// Вся ожидающая выполнения работа GPU уже завершена. Обновите отслеживаемые значения границы
	// до последнего переданного значения.
	for (UINT n = 0; n < c_frameCount; n++)
	{
...
void DX::DeviceResources::ValidateDevice()
{
	...
	// Сначала необходимо получить LUID для адаптера на момент создания устройства.
